...
def run_nist(prompt, response, category):
    with open("schemas/nistschema.json", "r", encoding="utf-8") as f:
        schema = json.load(f)

    with open(f"prompts/{category.lower()}_nist.txt", "r") as f:
    ...
    try:
        return json.loads(result_text)[0]["annotations"]["NIST RMF"]
    except:
        return []
