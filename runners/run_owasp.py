...
def run_owasp(prompt, response, category):
    with open("schemas/owaspschema.json", "r", encoding="utf-8") as f:
        schema = json.load(f)

    with open(f"prompts/{category.lower()}_owasp.txt", "r") as f:
    ...
    try:
        return json.loads(result_text)[0]["annotations"]["OWASP"]
    except:
        return []
